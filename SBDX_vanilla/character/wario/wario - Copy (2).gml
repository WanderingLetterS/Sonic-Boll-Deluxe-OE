...
draw_skintext(floor(x)-string_length(string(hsp))*4,floor(y)-32,machrun)
...
if (ckey) {
if (!jump && !water && !crouch && !carry && !bash && !machrun && !brakingmach && abs(hsp)>0.5) {
machrun=1
}
} else { if (machbash && abs(hsp)>0.5) {brakingmach=1 braketimer=31 machbash=0 machrun=0 machtime=0 playsfx(name+"skid")
	soundstop(name+"machbashstart") soundstop(name+"machbashloop") }
machrun=0}
...
} else {machtime=0 machbash=0 if (machbash && abs(hsp)>0.5) && !left && !right {{brakingmach=1 braketimer=31 machbash=0 machrun=0 machtime=0 playsfx(name+"skid")
	soundstop(name+"machbashstart") soundstop(name+"machbashloop") }} }
...
if (machbash && xsc!=h && h!=0 && !jump) {
	brakingmach=2 braketimer=31 machbash=0 machrun=0 machtime=0 playsfx(name+"skid")
	soundstop(name+"machbashstart") soundstop(name+"machbashloop")
}
if (machbash && h=0 && !jump && brakingmach!=2) {
	brakingmach=1 braketimer=31 machbash=0 machrun=0 machtime=0 playsfx(name+"skid")
	soundstop(name+"machbashstart") soundstop(name+"machbashloop")
}


if (braketimer=11) && !(left || right) {braketimer=0 brakingmach=0}


braketimer=max(0,braketimer-1)
if (brakingmach && !jump && braketimer=1) {
	/*
	if (brakingmach=2) {
		xsc=-xsc
		hsp=3*xsc
		if (ckey && (left || right) && xsc!=h) {machtime=1 machbash=1}
		brakingmach=0
	}
	if brakingmach=1 {brakingmach=0}
	*/
	if brakingmach=1 {
		if (ckey && (left || right) && xsc!=h) {
			xsc=-xsc
			hsp=3*xsc
			machtime=1 machbash=1
		}
		brakingmach=0
	}
	braketimer=0
...
if (brakingmach) {machrun=0 machbash=0 machtime=0 braketimer=0 brakingmach=0}
