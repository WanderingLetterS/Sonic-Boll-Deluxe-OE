#define spritelist
stand,wait,lookup,pose,walk,jump,bonk,fall,wall,dash,bow,stand2,wait2,lookup2,pose2,walk2,jump2,bonk2,fall2,wall2,dash2,bow2,standcarry,walkcarry,jumpcarry,fallcarry,bonkcarry,dashcarry,flagslide,climbing,dead,grind,run,spin,piping,pipingup,sidepiping,doorenter,doorexit
...
dodeathanim=funnytruefalse(playerskindat(p2,string(name)+" death animation")) //weird i have to do it like this
...
#define itemget
if ((!piped || monitem) && !hurt) { 
	if !(global.mplay>1 && flash) {
		if (type="fflower") {
			hurtplayer("item")             
		}
...
if (event="draw") {
	...
	if (bow) {
		...
		draw_sprite_part_ext(sheet,0,10+9*frame,87,8,8,round(x-4*xsc),round(y-4),xsc,1,$ffffff,1)
	}
	else if bloddrop {
...
if (useypos) {
if (fired && (sprite!="flagslide" || sprite!="climbing" || sprite!="grind" || sprite!="piping" || sprite!="pipingup" || sprite!="sidepiping" || sprite!="doorenter" || sprite!="doorexit" || sprite!="run")) {ypos=11}else ypos=0
} else ypos=0
...
if (instance_place(x,y,part)) && size!=1 hurtplayer(1)
