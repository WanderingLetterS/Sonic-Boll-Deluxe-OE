...
else if (crouch || revcrouch) {sprite="crouch"}
else if (jump) {
	if !(spindash) {
	if (onvine) 
	{
...
com_piping()
    ...
    if (!jump) {
		...
		if crouch {crouchframer=frame if sign(animf)!=-1 {animf=animf*-1} revcrouch=1 crouch=0}
    }
    mask_reset()
}
...
maxspd=(3.2 + !!size*0.25 + (size==5)*0.55 + (boost / 2.5) + (boostvar) + firedash/24+ water*0.1 + (spin*10))*wf

//There's an extra check in the hsp+= section of h!=0 to compensate!.
if (abs(hsp)>maxspd) {
	if (!spin && !(jump && (!fall || fall=10))) hsp=(abs(hsp)*2+maxspd)/3*sign(hsp)
    else if (abs(hsp) > 5) hsp = max(abs(hsp) - 0.005, 5)*sign(hsp)
}
...
if revcrouch {
	if (crouchframer>frame) crouchframer=frame
	//if floor(frame)=frn-1 crouchdont=1
    else if (frame=0) /*(frame+frs>crouchframer)*/ || (crouch || hsp!=0 || jump || up) {animf=animf*-1 crouchdont=0 crouchframer=0 revcrouch=0}
} else if crouchframer crouchframer=0
...
if !(jump && fall=10)
boostvar=inch(boostvar,0.5*boost*!jump*!spin,0.015)
if (boost) {
