...
show_message("Global Manager has been destroyed, the game will now crash.")
...
//update extensions
var i,list,savedepth;
i=0;
list = global.extensions_draw
repeat(ds_list_size(list)) {
    savedepth=depth;
    code_execute(ds_list_find_value(list,i));
    depth=savedepth
    i+=1;
}

if (global.showfps) {
    draw_systext(64,8 + (instance_exists(console) * 48),"FPS: "+string_format(fps_real,3,1))
}
