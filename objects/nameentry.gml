...
ta_load()
cheater = global.cheater + global.lowgrav + global.highgrav + global.fuzzy + global.debug + global.epic_speed_mode + (players[0].tick<180)
tas=""
if global.wastasing || global.tasing tas="tas_"
...
if (!cheater && string(pname)!="" && string(pname)!="0" && !(settings("Hi_im_dev") && (name == "ashura" || name == "super")) /*don't add unlockable dev times to the regular leaderboard*/) {
    ...
    for (j=4;j>=placement;j-=1) {
        ta_set("ta_time_"+tas+string(j+1)+global.currentlevel+l,ta_set("ta_time_"+tas+string(j)+global.currentlevel+l)) //time
        ta_set("ta_char_"+tas+string(j+1)+global.currentlevel+l,ta_set("ta_char_"+tas+string(j)+global.currentlevel+l)) //player name
        ta_set("ta_dev_"+tas+string(j+1)+global.currentlevel+l,ta_set("ta_dev_"+tas+string(j)+global.currentlevel+l)) //Add the dev icon at the right of the time!
        ta_set("ta_pname_"+tas+string(j+1)+global.currentlevel+l,ta_set("ta_pname_"+tas+string(j)+global.currentlevel+l)) //You.
    }
...
if (!cheater && string(pname)!="" && string(pname)!="0") {
    ...
    for (j=4;j>=placement;j-=1) {
        ta_set("ta_time_"+tas+string(j+1)+global.currentlevel+l+name,ta_set("ta_time_"+tas+string(j)+global.currentlevel+l+name)) //time
        ta_set("ta_char_"+tas+string(j+1)+global.currentlevel+l+name,ta_set("ta_char_"+tas+string(j)+global.currentlevel+l+name)) //player name
        ta_set("ta_dev_"+tas+string(j+1)+global.currentlevel+l+name,ta_set("ta_dev_"+tas+string(j)+global.currentlevel+l+name)) //Add the dev icon at the right of the time!
        ta_set("ta_pname_"+tas+string(j+1)+global.currentlevel+l+name,ta_set("ta_pname_"+tas+string(j)+global.currentlevel+l+name)) //You.
    }
