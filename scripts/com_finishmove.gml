if (!dead) {
    ...
    //previously in vine_climbing
    if (up && !down && !onvine && !hurt) {
    cx=floor(x/16)*16+8
    cy=floor(y/16)*16+8
    onvine=position_meeting(cx,cy,vine)
    if (onvine) {fall=69 jump=1} //nice
}
    ...
    coll=collision(0,0)
    if (coll) {
        if (coll.object_index != phaser && !object_is_ancestor(coll.object_index,phaser)) { //DUDE
            expulsion()
        }
    }
    else {
