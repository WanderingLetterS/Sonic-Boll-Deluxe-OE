...
if (other!=noone && other!=0) {
    if ((other.object_index==projectile || other.object_index==damager || other.object_index==player) && object_index==player && global.gamemode=="coop") return 0
}
...
if (harmtype="pvp" && !super && !shielded) {
    ...
    else if (size=6) {i=instance_create(x,y,itemdrop) {if (lock) {i.vspeed-=2} i.heavened=heavened i.p2=p2 i.type="btroot"}}
    else if (size=7) {i=instance_create(x,y,itemdrop) {if (lock) {i.vspeed-=2} i.heavened=heavened i.p2=p2 i.type="glui"}}
