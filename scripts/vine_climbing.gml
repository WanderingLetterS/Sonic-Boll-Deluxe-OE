...
if (fall=69) {
    ...
    vinecoll=collision(0,0)
    if (vinecoll && vinecoll.object_index != phaser && !object_is_ancestor(vinecoll.object_index,phaser)) {
        x=gobackx
        y=gobacky
    }

    if (down && position_meeting(ncx,ncy+8,collider)) {fall=0 onvine=0}
