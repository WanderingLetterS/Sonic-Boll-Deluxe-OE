...
if (dr.copy_layer == dr.current_layermode) {
...
with (drawregion) {
    ...
    var obj;
    switch(copy_layer) {
        case 0: obj = lemongrab.waters[region] break;
        case 1: obj = deity break;
        case 2: obj = lemongrab.semis[region] break;
        case 3: obj = lemongrab.backs[region] break;
    }

    with (obj) {event_user(0) selected=0 depth=1}

    for (i=1;i<=cbuffer[0];i+=1) if (lemonhappy()) {
        o=instance_create(cbuffer[i, 0]+curx-copyx,cbuffer[i, 1]+cury-copyy,obj)
        o.obj =cbuffer[i, 2]
        o.spr =cbuffer[i, 3]
        o.off =cbuffer[i, 4]
