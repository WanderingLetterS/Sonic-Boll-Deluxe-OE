...
if (string_pos("<",str) && string_pos(">",str)) {
    ...
    if (spec!=-1) {
        movesetpage+=1
        if (spec) {
            global.pagespec[argument[0],spec-1]=movesetpage
            findspec[movesetpage] = spec-1
        }
        exit
    }
}
...
if (is_real(movelistcache[movesetpage])) movelistcache[movesetpage]=""
global.movelist[argument[0],movesetpage]+=replacebuttonnames(str)+"#" movelistcache[movesetpage]+=str+"#"
